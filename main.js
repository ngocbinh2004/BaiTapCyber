...
function calculateDiscount() {
  const amount = parseFloat(
    document.getElementById("purchaseAmountInput").value
  );
  ...
  if (!isNaN(amount) && amount > 500) {
    ...
    pThongBaoChietKhau.textContent = `Bạn được giảm 20%. Số tiền sau khi giảm là: $${finalPrice.toFixed(
      2
    )}`;
    pThongBaoChietKhau.style.color = "green";
  } else if (!isNaN(amount)) {
    pThongBaoChietKhau.textContent =
      "Bạn không đủ điều kiện để nhận chiết khấu";
...
function checkPassword() {
  ...
  password.length >= 8
    ? ((passwordResult.textContent = "Mật khẩu đủ mạnh"),
      (passwordResult.style.color = "green"))
    : ((passwordResult.textContent = "Mật khẩu quá yếu, cần ít nhất 8 ký tự"),
