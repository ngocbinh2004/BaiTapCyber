...
function calculateAdmission() {
  ...
  const priorityObject = parseInt(
    document.getElementById("priorityObject").value
  );
  ...
  if (!(region in regionScores) || !(priorityObject in objectScores)) {
    document.getElementById("pThongBaoTuyenSinh").innerText =
      "Khu vực hoặc đối tượng không hợp lệ.";
    return;
  }
  ...
  if (totalScore <= 0 || finalScore >= cutoffScore) {
    document.getElementById(
      "pThongBaoTuyenSinh"
    ).innerText = `Đậu! Tổng điểm của bạn là: ${finalScore}`;
  } else {
    document.getElementById(
      "pThongBaoTuyenSinh"
    ).innerText = `Rớt! Tổng điểm của bạn là: ${finalScore}`;
  }
}
...
function calculateElectricity() {
  ...
  document.getElementById(
    "pThongBaoTienDien"
  ).innerText = `Khách hàng ${nameTienDien} phải trả số tiền: ${totalCost.toLocaleString()} VND cho ${kw} Kw.`;
}
...
function calculateTax() {
  ...
  if (taxableIncome <= 0) {
    document.getElementById(
      "result"
    ).innerText = `${nameThueCaNhan}, bạn không phải nộp thuế vì thu nhập chịu thuế của bạn là ${taxableIncome.toFixed(
      2
    )} triệu VND.`;
    return;
  }
  ...
  document.getElementById(
    "result"
  ).innerText = `${nameThueCaNhan}, tổng thuế thu nhập cá nhân phải nộp là: ${tax.toLocaleString()} triệu VND cho thu nhập chịu thuế là ${taxableIncome.toFixed(
    2
  )} triệu VND.`;
}
